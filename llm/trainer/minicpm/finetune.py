...
class SupervisedDataset(Dataset):
    ...
    def preprocessing(self, example):
        ...
        # 全部长度包括 : bos_token_id + user_tokens + content_ids_input + assistant_tokens + content_ids_label
        # 总长度 = model_max_length
        # content_ids_label 是一定要保证都包含在内的
        # input_ids_upper_len = model_max_length - (bos_token_id + user_tokens + assistant_tokens + content_ids_label + eos_token_id)
        msg = {}
        ...
        for message in example["messages"]:
            ...
            if role == "user":
                ...
                #     + content_ids
                # )
                msg["output_ids"] = content_ids

        

        # input_ids_upper_len = self.model_max_length - (len(self.tokenizer.bos_token_id) + len(self.user_tokens) + len(self.assistant_tokens) + len(msg["output_ids"]) + len(self.tokenizer.eos_token_id))
        input_ids_upper_len = self.model_max_length - (1 + len(self.user_tokens) + len(self.assistant_tokens) + len(msg["output_ids"]) + 1)

        # 判断是否需要cutoff
        input_ids_delta = input_ids_upper_len - len(msg["input_ids"])
        if input_ids_delta >= 0:
            # 可以容纳
            input_ids += self.user_tokens + msg["input_ids"]
            label_ids += [self.ignore_index] * len(self.user_tokens) + [
                self.ignore_index
            ] * len(msg["input_ids"])
            ...
            # 容不下
            input_ids += self.user_tokens + msg["input_ids"][: input_ids_upper_len]
            label_ids += [self.ignore_index] * len(self.user_tokens) + [
                self.ignore_index
            ] * len(msg["input_ids"][: input_ids_upper_len])


        ##
        input_ids += self.assistant_tokens + msg["output_ids"]
        label_ids += (
            [self.ignore_index] * len(self.assistant_tokens)
            + msg["output_ids"]
        )
        ...
        if input_ids_delta > 0:
            ...
            label_ids += [self.ignore_index] * (input_ids_delta)
            attention_mask += [0] * (input_ids_delta)
